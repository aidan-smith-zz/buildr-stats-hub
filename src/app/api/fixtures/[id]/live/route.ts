...
import { resolveMatchStatsForFixture, type MatchStatsSnapshot } from "@/lib/matchStats";
...
type FixtureForLive = {
  ...
  homeTeamId: number;
  awayTeamId: number;
  homeTeam: { apiId: string | null };
  awayTeam: { apiId: string | null };
};

async function matchStatsForResponse(
  fixture: FixtureForLive,
  now: Date,
  refreshFromApi: boolean,
): Promise<{ home: MatchStatsSnapshot; away: MatchStatsSnapshot } | undefined> {
  const kickoff = new Date(fixture.date);
  if (kickoff > now) return undefined;
  const pair = await resolveMatchStatsForFixture(
    {
      id: fixture.id,
      apiId: fixture.apiId,
...
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  return withPoolRetry(async () => {
    ...
    if (cacheSaysEnded && cached) {
      const matchStats = await matchStatsForResponse(fixture, now, false);
      return NextResponse.json(
        liveBody(
          {
            live: true,
