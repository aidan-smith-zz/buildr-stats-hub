...
export default async function LeagueBttsPage({ params }: Props) {
  const slug = normalizeSlug((await params).league);
  const leagueId = standingsSlugToLeagueId(slug);
  if (leagueId === undefined) notFound();

  const [data, crestUrl] = await Promise.all([
    getLeagueBttsMarketData(leagueId),
  ...
  const breadcrumbItems = [
    ...
    { href: `/leagues/${leagueSlug}/markets/btts`, label: "BTTS stats" },
  ];
  ...
  const faqJsonLd = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: `What are ${leagueName} BTTS stats?`,
        acceptedAnswer: {
          "@type": "Answer",
          text: `BTTS (both teams to score) stats for ${leagueName} show how often league matches this season have seen both sides score at least once. You can also see which teams have the highest BTTS percentage.`,
        },
      },
      {
        "@type": "Question",
        name: `How can I use ${leagueName} BTTS stats for betting?`,
        acceptedAnswer: {
          "@type": "Answer",
          text: `Use the league-wide BTTS percentage and team BTTS rankings to spot matches where both teams scoring is common. Combine this with fixture odds and team pages on statsBuildr when building BTTS and bet builder selections.`,
        },
      },
    ],
  };
  ...
  const itemListJsonLd =
    data.teams.length > 0
      ? {
          "@context": "https://schema.org",
          "@type": "ItemList",
          name: `${leagueName} BTTS team stats ${season}`,
          description: `Team BTTS (both teams to score) percentages for ${leagueName} in the ${season} season.`,
  ...
  const webPageJsonLd = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: `${leagueName} BTTS stats ${season} | Both teams to score trends`,
    description: `League-wide BTTS (both teams to score) trends for ${leagueName}: overall BTTS rate, team BTTS percentages and recent BTTS results for ${season}.`,
    url: `${BASE_URL}/leagues/${leagueSlug}/markets/btts`,
  };
      ...
      <main className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <h1 className="mt-1 text-xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-2xl">
                  {leagueName} BTTS stats
                </h1>
                <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                  League-wide both teams to score trends for {leagueName}: overall BTTS rate, team BTTS percentages and recent BTTS results.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="mb-6 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-50 sm:text-base">
            League BTTS this season
          </h2>
          <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
            How often both teams scored in {leagueName} matches we&apos;ve tracked for the {season} season.
          </p>
          {data.totalMatches === 0 ? (
            <p className="mt-3 text-sm text-neutral-500 dark:text-neutral-500">
              We don&apos;t have BTTS data for this league yet.
            </p>
          ) : (
            <div className="mt-3 flex items-baseline gap-2">
              <span className="text-3xl font-bold tabular-nums text-neutral-900 dark:text-neutral-50">
                {data.bttsPct != null ? data.bttsPct.toFixed(1) : "—"}%
              </span>
              <span className="text-sm text-neutral-500 dark:text-neutral-500">
                ({data.bttsYes} of {data.totalMatches} matches)
              </span>
            </div>
          )}
        </section>
        ...
        <section className="mb-6 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-50 sm:text-base">
            Team BTTS percentages
          </h2>
          <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
            BTTS (both teams to score) percentage by team in {leagueName}. Sorted by highest BTTS rate.
          </p>
          {data.teams.length === 0 ? (
            <p className="mt-3 text-sm text-neutral-500 dark:text-neutral-500">
              We don&apos;t have team BTTS data for this league yet.
            </p>
          ) : (
        ...
        <section className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-50 sm:text-base">
            Recent BTTS results in {leagueName}
          </h2>
          <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
            Recent league matches and whether both teams scored.
          </p>
          {data.recentFixtures.length === 0 ? (
            <p className="mt-3 text-sm text-neutral-500 dark:text-neutral-500">
              No recent league fixtures with BTTS data yet.
            </p>
          ) : (
        ...
        {data.teams.length > 0 ? (
          <section className="mt-6 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-50 sm:text-base">
              Top 5 BTTS teams in {leagueName}
            </h2>
            <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
              Teams with the highest share of league matches where both teams scored this season.
            </p>
            <ul className="mt-3 space-y-2 text-sm">
                ...
                >
                  ...
                  <div className="text-right text-xs text-neutral-600 dark:text-neutral-400">
                    ...
                    <p className="tabular-nums">
                      {team.bttsYes} of {team.matches} games
                    </p>
                  </div>
                </li>
              ))}
        ...
        <div className="mt-8 flex justify-center">
          <ShareUrlButton
            title={`${leagueName} BTTS stats ${season} | statsBuildr`}
            text={`${leagueName} both teams to score stats for the ${season} season. League BTTS rate, team BTTS percentages and recent BTTS results on statsBuildr.`}
