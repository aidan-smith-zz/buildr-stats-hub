...
export default async function LeagueCornersPage({ params }: Props) {
  const slug = normalizeSlug((await params).league);
  const leagueId = standingsSlugToLeagueId(slug);
  if (leagueId === undefined) notFound();

  const [data, crestUrl] = await Promise.all([
    getLeagueCornersMarketData(leagueId),
  ...
  const breadcrumbItems = [
    ...
    { href: `/leagues/${leagueSlug}/markets/corners`, label: "Team corners stats" },
  ];
  ...
  const faqJsonLd = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: `What are ${leagueName} team corners stats?`,
        acceptedAnswer: {
          "@type": "Answer",
          text: `Team corners stats for ${leagueName} show how often each team wins over lines like 3.5, 4.5 and 5.5 corners in league matches. You can also see which teams regularly rack up high corner counts.`,
        },
      },
      {
        "@type": "Question",
        name: `How can I use ${leagueName} team corners stats for betting?`,
        acceptedAnswer: {
          "@type": "Answer",
          text: `Use the league-wide over 3.5, 4.5 and 5.5 team corners rates and team rankings to find sides that consistently win many corners. Combine this with fixture odds and team pages on statsBuildr when building team corners legs in your bet builder.`,
        },
      },
    ],
  };
  ...
  const itemListJsonLd =
    data.teams.length > 0
      ? {
          "@context": "https://schema.org",
          "@type": "ItemList",
          name: `${leagueName} team corners stats ${season}`,
          description: `Team over 3.5 corners percentages for ${leagueName} in the ${season} season.`,
  ...
  const webPageJsonLd = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: `${leagueName} team corners stats ${season} | Over 3.5, 4.5, 5.5`,
    description: `League-wide team corners trends for ${leagueName}: over 3.5, over 4.5 and over 5.5 team corners rates, team rankings and recent high corners games for ${season}.`,
    url: `${BASE_URL}/leagues/${leagueSlug}/markets/corners`,
  };
      ...
      <main className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <h1 className="mt-1 text-xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-2xl">
                  {leagueName} team corners stats
                </h1>
                <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                  League-wide team corners trends for {leagueName}: over 3.5, over 4.5 and over 5.5 team corners rates, team rankings and recent high corners results.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="mb-6 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-50 sm:text-base">
            League over 3.5, 4.5 and 5.5 team corners this season
          </h2>
          <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
            How often <strong>individual team corners</strong> lines of 3.5, 4.5 and 5.5 have landed across {leagueName} team-games we&apos;ve tracked in {season}
            (each row is one team&apos;s corners total in a match, not both sides combined).
          </p>
          {data.totalRows === 0 ? (
            <p className="mt-3 text-sm text-neutral-500 dark:text-neutral-500">
              We don&apos;t have team corners data for this league yet.
            </p>
          ) : (
            <div className="mt-3 grid gap-4 sm:grid-cols-3">
              <div className="rounded-xl border border-neutral-200 bg-neutral-50/60 p-3 dark:border-neutral-800 dark:bg-neutral-900/60">
                <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">Over 3.5 team corners</p>
              ...
              <div className="rounded-xl border border-neutral-200 bg-neutral-50/60 p-3 dark:border-neutral-800 dark:bg-neutral-900/60">
                <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">Over 4.5 team corners</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-neutral-900 dark:text-neutral-50">
                  {data.over45Pct != null ? `${data.over45Pct.toFixed(1)}%` : "—"}
                </p>
                <p className="text-xs text-neutral-500 dark:text-neutral-500">
                  {data.over45} of {data.totalRows} team-games
                </p>
              </div>
              <div className="rounded-xl border border-neutral-200 bg-neutral-50/60 p-3 dark:border-neutral-800 dark:bg-neutral-900/60">
                <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">Over 5.5 team corners</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-neutral-900 dark:text-neutral-50">
                  {data.over55Pct != null ? `${data.over55Pct.toFixed(1)}%` : "—"}
                </p>
                <p className="text-xs text-neutral-500 dark:text-neutral-500">
                  {data.over55} of {data.totalRows} team-games
                </p>
              </div>
            </div>
          )}
        ...
        <section className="mb-6 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-50 sm:text-base">
            Team over 3.5 corners percentages
          </h2>
          <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
            Percentage of league matches where each team&apos;s <strong>own corners total</strong> went over 3.5. Sorted by highest over 3.5 rate.
          </p>
          {data.teams.length === 0 ? (
            <p className="mt-3 text-sm text-neutral-500 dark:text-neutral-500">
              We don&apos;t have team corners data for this league yet.
        ...
        <section className="mb-6 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-50 sm:text-base">
            Recent high corners performances in {leagueName}
          </h2>
          <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
            Recent league team-games where a single team recorded notable corners totals (corners for that team only, not both sides).
          </p>
          {data.recentRows.length === 0 ? (
            <p className="mt-3 text-sm text-neutral-500 dark:text-neutral-500">
              No recent league fixtures with team corners data yet.
            </p>
          ) : (
        ...
        {data.teams.length > 0 ? (
          <section className="mt-6 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-50 sm:text-base">
              Top 5 over 3.5 corners teams in {leagueName}
            </h2>
            <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-400">
              Teams with the highest share of league matches where their team corners went over 3.5 this season.
            </p>
            <ul className="mt-3 space-y-2 text-sm">
                ...
                >
                  ...
                  <div className="text-right text-xs text-neutral-600 dark:text-neutral-400">
                    ...
                    <p className="tabular-nums">
                      {team.over35} of {team.matches} team-games
                    </p>
                  </div>
                </li>
              ))}
        ...
        <div className="mt-8 flex justify-center">
          <ShareUrlButton
            title={`${leagueName} team corners stats ${season} | statsBuildr`}
            text={`${leagueName} team corners stats for the ${season} season: over 3.5, 4.5 and 5.5 team corners rates, team rankings and recent high corners results on statsBuildr.`}
