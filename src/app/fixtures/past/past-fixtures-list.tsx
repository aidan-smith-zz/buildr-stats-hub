...
import type { PastFixturesByDate } from "@/lib/fixturesService";
import type { FixtureSummary } from "@/lib/statsService";

const FINISHED_STATUSES = new Set(["FT", "AET", "PEN", "ABD", "AWD", "WO", "CAN"]);
...
function formatKoTime(date: Date | string): string {
...
type Props = {
  byDate: PastFixturesByDate;
};

export function PastFixturesList({ byDate }: Props) {
    ...
    return (
      <div className="mt-10 rounded-2xl border border-neutral-200 bg-white p-10 text-center shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
        <p className="text-sm text-neutral-600 dark:text-neutral-400">
          No past fixtures in the last 14 days. Fixtures appear here after they have been played and warmed.
        </p>
      </div>
    );
  }
            ...
            return (
              <section key={dateKey}>
                ...
                <ul className="space-y-2">
                  {sorted.map((f: FixtureSummary, index: number) => {
                    const home = f.homeTeam.shortName ?? f.homeTeam.name;
                    const away = f.awayTeam.shortName ?? f.awayTeam.name;
                    ...
                    const isFinished = f.statusShort != null && FINISHED_STATUSES.has(f.statusShort);
                    const itemKey = `${dateKey}-${f.id}-${home}-${away}-${index}`;
                      ...
                      <li key={itemKey} className="relative">
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="flex items-center gap-2 text-xs text-neutral-500 dark:text-neutral-400 sm:text-sm">
                              {koTime}
                              {isFinished && (
                                <span
                                  className="rounded bg-neutral-200 px-1.5 py-0.5 text-[10px] font-medium text-neutral-600 dark:bg-neutral-600 dark:text-neutral-200 sm:text-xs"
